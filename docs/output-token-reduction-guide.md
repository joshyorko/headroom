...
- Anthropic/Claude uses verbosity steering plus effort routing.
- OpenAI/Codex uses stable verbosity steering on chat completions, Responses,
  and Codex WebSocket request paths.
- OpenAI/Codex effort routing is conservative: Headroom only lowers compatible
  effort fields the client already sent, such as `reasoning_effort` or
  `reasoning.effort`. It never injects new effort fields.

Output shaping reduces output tokens by steering the request before generation.
It cannot trim output that has already been generated.
